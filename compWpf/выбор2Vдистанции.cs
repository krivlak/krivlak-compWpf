...
namespace compWpf
{
    public partial class выбор2Vдистанции : Form
    {
        public выбор2Vдистанции()
        {
            InitializeComponent();
            this.Top = 0;
     ...
     //   List<Form> formList = new List<Form>();
            ...
            try
            {
                ...
                foreach (var gg in de.виды.OrderBy(n => n.порядок))
                {
                    ...
                    foreach (var mm in de.дистанции.Where(n => n.вид == gg.вид).Where(n => n.слет == клСлет.слет).OrderBy(n => n.порядок))
                    {
                        ...
                        //   int судов = mm.суда.Count();
                        int экипажей = mm.экипажи.Count();
                        TreeNode node1 = node.Nodes.Add(mm.наимен + "   экипажей " + экипажей.ToString());
                        node1.Tag = mm;
      ...
      //          клДистанция.окно_выбора = this;
            ...
            Activated += Выбор2Vдистанции_Activated;
        }

        private void Выбор2Vдистанции_Activated(object sender, EventArgs e)
        {
            treeView1.Focus();
        }
